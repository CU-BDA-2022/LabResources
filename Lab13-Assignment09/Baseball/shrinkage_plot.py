...
def shrinkage_plot(x_vals, pdf_vals, x_true, x_ml, x_post, xlabel,
                   log_x=False, log_y=False, pdf_label=None, legend=True):
    """
    ...
    # True:
    if log_x and log_y:
        ax_pdf.loglog(x_vals, pdf_vals, 'b-', lw=2, label=pdf_label)
    elif log_x:
        ax_pdf.semilogx(x_vals, x_vals*pdf_vals, 'b-', lw=2, label=pdf_label)
    else:
        ax_pdf.plot(x_vals, pdf_vals, 'b-', lw=2, label=pdf_label)
    ...
    ax_pts.text(.02, y_ml+.015, 'MLEs', **tdict)
